...
const MODES = {
  ...
  guide: { name:'Проводник',  tag:'Кооператив',      desc:'Слово знает только один. Говорить ему нельзя — ведёт кнопками.' }
};
...
function roundFor(room, c){
  ...
  return { type:'round', running:G.running, stage:G.stage, mode:G.mode,
           ...
           secs: G.running ? Math.max(0, Math.ceil((G.endsAt - Date.now())/1000)) : 0,
...
function tellYou(room, c){
  write(c, { type:'you', cid:c.cid, role: room.roleOf.get(c.cid) || null,
             ...
             room: room.code, stage: room.G.stage, mode: room.G.mode });
}
...
function startRound(room){
  ...
  G.endsAt = Date.now() + ROUND_S * 1000;
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/stream') {
    ...
    write(client, { type:'hello', w:W, h:H, colors:COLORS, limits:LIMITS,
                    roundSecs:ROUND_S, modes:MODES, room:room.code });
