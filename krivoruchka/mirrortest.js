...
const http=require('http'); const BASE='http://localhost:3000'; const got={};
function stream(t,cid){got[t]=[];return new Promise(r=>{http.get(BASE+'/stream?cid='+cid,res=>{res.setEncoding('utf8');
res.on('data',ch=>ch.split('\n').forEach(l=>{if(l.startsWith('data: ')){try{got[t].push(JSON.parse(l.slice(6)))}catch(e){}}}));r(res);});});}
function post(b){return new Promise(r=>{const d=JSON.stringify(b);const q=http.request(BASE+'/msg',{method:'POST',
