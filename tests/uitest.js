...
(async () => {
  const d1 = await open(); await wait(900);
  const w1 = d1.window, doc1 = w1.document;

  ok('без кода показан экран входа', doc1.getElementById('enter').classList.contains('on'));
  ok('есть кнопка создания комнаты', !!doc1.getElementById('createBtn'));

  click(w1, doc1.getElementById('createBtn'));
  await wait(1200);
  const room = doc1.getElementById('roomCode').textContent.trim();
  ok('комната создана и код показан', room.length === 4);
  ok('код попал в адрес страницы', decodeURIComponent(w1.location.search).includes(room));
  ...
  const d2 = await open('?r=' + encodeURIComponent(room)); await wait(1300);
  const w2 = d2.window, doc2 = w2.document;
  ok('второй попал в ту же комнату', doc2.getElementById('roomCode').textContent.trim() === room);
